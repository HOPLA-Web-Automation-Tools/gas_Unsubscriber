...
var frequency = userProperties.getProperty("frequency") || 10;
  ...
  for (var t in threads) {
    ...
    if (urls) {
      ...
      if (urls) {
        ...
        var subj = urls[2].match(/subject=([\w ]+)/i);
        if (subj) {
          var split = urls[2].split('?');
          if (split.length > 1) {
